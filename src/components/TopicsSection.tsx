...
export function TopicsSection() {
  ...
  return (
    <section id="temy" className="bg-agora-cream py-16 pb-24 border-t border-[#e2d5c8]">
            ...
            return (
              <Reveal key={topic.name} delay={(i % 5) * 60} className="flip-card group aspect-square [perspective:1000px]">
                <div className="flip-inner relative w-full h-full transition-transform duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                  <div className={`absolute inset-0 [backface-visibility:hidden] ${frontBg} ${frontText} p-6 flex flex-col justify-start`}>
                    <span className="font-serif text-2xl mb-1 block">{i + 1}.</span>
                    <h3 className="font-serif text-2xl font-bold leading-tight">{topic.name}</h3>
                    <p className="front-question text-sm font-light leading-snug mt-2 opacity-70">{topic.question}</p>
                  </div>
                  <div className={`flip-back absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] ${backBg} ${backText} p-6 flex items-center justify-center text-center`}>
                    <p className="font-serif text-lg italic leading-snug">{topic.question}</p>
                  </div>
                </div>
              </Reveal>
            );
